...
- **No PRD/plan references in shipped code**: Specification artifacts under
  `{SPECS_DIR}/` (PRD, issues, plans) are transient working files — typically
  gitignored and never committed to the repository.
  Code comments must describe what the code does — never reference the PRD or
  the implementation plan, and never embed their internal identifiers
  (success-criteria IDs `SC-001`, user-story IDs `User Story 1` or `US1`,
  issue IDs `1-AFK`/`1-HITL`, acceptance-criteria `AC1`) in comments, test
  names, identifiers, or commit messages.
  The plan's task structure and `[x]` progress markers already track the
