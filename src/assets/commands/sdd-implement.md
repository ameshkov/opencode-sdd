...
- **No spec/plan references in shipped code**: Specification artifacts under
  `{SPECS_DIR}/` (the spec) are transient working files — typically gitignored
  and never committed to the repository.
  Code comments must describe what the code does — never reference the spec or
  the implementation plan, and never embed their internal identifiers
  (functional-requirement references, acceptance-scenario labels, task IDs such
  as `Task 1.2`) in comments, test names, identifiers, or commit messages.
  The spec's task structure and `[x]` progress markers already track the
