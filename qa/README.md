...
| `qa/docker/` | In-container scripts (baked into the image at `/app`): provisioning, scratch project, config wiring, smoke test |
...
patch never touch the host. The host runs only docker (plus, once, an
OpenRouter key) and holds this repo.
...
| Outbound HTTPS to `api.openrouter.ai` | host (docker) | the gateway forwards every request there; checked by `check-deps.sh` |
...
qa/scripts/setup/qa-up.sh      # build + start gateway and workspace, wait for health,
                               # provision the provider (key from qa/.env)
...
- Host-side access (the bifrost Web UI and host-side smoke tests) is an
  opt-in override: `QA_HOST_PORT=8080 qa/scripts/setup/qa-up.sh` (or put
  `QA_HOST_PORT=8080` in `qa/.env`; or, as plain compose: `docker
  compose -f qa/docker-compose.yml -f qa/docker-compose.ports.yml up
  -d`). The published host port is `BIFROST_PORT` (default `8080`; set
  it to another value when 8080 is taken). Running the scripts without
  `QA_HOST_PORT` always restores the hermetic state.
- macOS sandbox VM: even with the ports override, the published port
  ...
  `http://192.168.64.1:<port>` instead of localhost. Inside the workspace
...
2. Ensure `qa/scripts/setup/check-deps.sh` passes and `qa/scripts/setup/qa-up.sh`
   reports the stack healthy (gateway healthy, workspace ready).
3. Open the TUI: `qa exec 'opencode --log-level DEBUG --print-logs'`.
4. Enumerate the group's cases with `pnpm qa:run --feature <file> --list`.
5. Run the cases (verbatim from the runner), then verify every asserted
   step before recording a verdict in the runner.
6. Copy artifacts out with `docker compose cp` for the evidence folder
   (section 7) and record the outcome in the runner's report.
...
- terminal captures or screenshots of the TUI;
...
| Symptom | Fix |
| --- | --- |
| `check-deps.sh` fails on OpenRouter reachability | the docker host needs outbound HTTPS to `api.openrouter.ai`; fix the network/firewall, not the stack |
...
| After changing `qa/bifrost/models.tsv` nothing changed | the provider config persists in the gateway data volume: `docker volume rm opencode-sdd-qa_bifrost-data`, then `qa-up.sh` (re-provisions) |
